					...
					if (Physics.Raycast (point + Vector3.up * 50, Vector3.down, out hit)) {
						...
						if (terrain != null) {
							...
							if (terrainHeights.ContainsKey (terrain)) {
								...
								heightmapData = terrainData.GetHeights (0, 0, terrain.terrainData.heightmapResolution, terrain.terrainData.heightmapResolution);
								terrainHeights.Add (terrain, heightmapData);
								oldHeightmapData = terrainData.GetHeights (0, 0, terrain.terrainData.heightmapResolution, terrain.terrainData.heightmapResolution);
								oldterrainHeights.Add (terrain, oldHeightmapData);
							}
							Vector3 heightmapPos = hit.point - terrain.transform.position;
							float terrainToheight = (1 / terrainData.size.x * terrain.terrainData.heightmapResolution);
							float terrainTowidth = (1 / terrainData.size.z * terrain.terrainData.heightmapResolution);
							heightmapPos.x = heightmapPos.x * terrainToheight;
							heightmapPos.z = heightmapPos.z * terrainTowidth;
							heightmapPos.x = Mathf.Clamp (heightmapPos.x, 0, terrain.terrainData.heightmapResolution - 1);
							heightmapPos.z = Mathf.Clamp (heightmapPos.z, 0, terrain.terrainData.heightmapResolution - 1);
